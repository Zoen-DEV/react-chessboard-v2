...
interface ChessboardProviderContext {
  ...
  altLeftClickArrowColor: ChessboardProps["altLeftClickArrowColor"];
  shiftLeftClickArrowColor: ChessboardProps["shiftLeftClickArrowColor"];
  shiftRightClickArrowColor: ChessboardProps["shiftRightClickArrowColor"];
  shiftAltLeftClickArrowColor: ChessboardProps["shiftAltLeftClickArrowColor"];
  shiftAltRightClickArrowColor: ChessboardProps["shiftAltRightClickArrowColor"];
  ...
  (
    {
      ...
      altLeftClickArrowColor = "rgb(0,48,136)",
      shiftLeftClickArrowColor = "rgb(21,120,27)",
      shiftRightClickArrowColor = "rgb(136,32,32)",
      shiftAltLeftClickArrowColor = "rgb(0,48,136)",
      shiftAltRightClickArrowColor = "rgb(255,170,0)",
      customBoardStyle,
      customNotationStyle,
    ...
    const [currentArrowColor, setCurrentArrowColor] = useState<
      string | undefined
    >(customArrowColor);
    ...
    const [currentLeftClickDown, setCurrentLeftClickDown] = useState<
      Square | undefined
    >();
    ...
    const [isShiftAltKeyDown, setIsShiftAltKeyDown] = useState(false);
    ...
    useEffect(() => {
      document.addEventListener("keydown", handleSetKeyDown, false);
      document.addEventListener("keyup", handleSetKeyUp, false);

      if (listenerContainerRef && listenerContainerRef?.current) {
        listenerContainerRef.current.focus();
      }

      return () => {
        document.removeEventListener("keydown", handleSetKeyDown, false);
        document.removeEventListener("keyup", handleSetKeyUp, false);
      };
    }, []);

    function handleSetKeyDown(event: KeyboardEvent): void {
      ...
      if (event.shiftKey && event.altKey) {
        setIsShiftAltKeyDown(true);
      }
    }

    function handleSetKeyUp() {
      setIsAltKeyDown(false);
      setIsShiftKeyDown(false);
      setIsShiftAltKeyDown(false);
    }
    ...
    function handleSetPosition(
      ...
      if (onPieceDrop.length) {
        ...
        if (!isValidMove) {
          ...
          clearArrows();
          setCurrentMoveInfo({ fromSquare: sourceSq, toSquare: targetSq });
        }
      } else {
        // delete source piece
        clearArrows();
    ...
    function onLeftClickDown(square: Square) {
      setCurrentLeftClickDown(square);
    }

    function onLeftClickUp(square: Square) {
      if (currentLeftClickDown) {
        // same square, don't draw an arrow, but do clear premoves and run onSquareLeftClick
        setCurrentLeftClickDown(undefined);
        if (currentLeftClickDown === square) {
          clearPremovesOnRightClick && clearPremoves(false);
          onSquareLeftClick(square);
          return;
        }
      } else {
        setCurrentLeftClickDown(undefined);
    ...
    function clearCurrentLeftClickDown() {
      setCurrentLeftClickDown(undefined);
    }
